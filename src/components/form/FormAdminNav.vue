<template>
  <VNavigationDrawer color="surface-light" v-model="drawer" expand-on-hover :rail="rail" class="nav" width="270">
    ...
    <div class="pt-10 pb-7 bg-background" style="position: sticky; top: 0; z-index: 99">
      ...
      <VScaleTransition leave-absolute>
        <div v-if="!rail">
          <p class="text-center mt-5 font-weight-semibold">{{ dataUser.authSession?.user.name }}</p>
          <p class="text-center text-body-2 text-medium-emphasis mt-1">
            <a href="mailto:joxmal123@gmail.com">{{ dataUser.authSession?.user.role }}</a>
          </p>
        </div>
      </VScaleTransition>
    </div>
    ...
    <VList nav>
      <template v-for="(m, i) in menu" :key="i">
        ...
        <VListGroup :value="m.title"
          v-if="m.items && !((m.title === 'Crear/Editar' || m.title === 'Administrar') && usePersistedStore().authSession?.user.role === 'user')">
...
const menu = [
  { title: "Panel", icon: "solar:home-angle-2-line-duotone", to: "/admin/dashboard" },
  {
    title: "Crear/Editar",
    icon: "solar:box-line-duotone",
    items: [
      { title: "Agencia", to: "/admin/dashboard/gestion/agencia" },
      { title: "Tasa Dolar", to: "/admin/dashboard/gestion/tasaDolar" },
